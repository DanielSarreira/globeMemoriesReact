...
async function main() {
  ...
  const r = await cdp.send('Runtime.evaluate', { expression: `(() => {
    ...
    const children = Array.from(layout.children).map(c => ({ class: c.className.split(' ')[0], w: c.getBoundingClientRect().width }));
    const card = document.querySelector('.gm-travel-card');
    return {
      layoutChildren: children,
      card: card ? { w: card.getBoundingClientRect().width, parent: card.parentElement.className } : null,
    };
