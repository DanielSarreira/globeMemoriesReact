// Screenshot of the redesigned modal v3
...
(async () => {
  ...
  // Pick first card + Continuar
  await send('Runtime.evaluate', {
    expression: `(() => {
      const cards = document.querySelectorAll('.my-travels-type-modal__body button');
      const btn = [...cards].find((b) => /Destino/.test(b.innerText));
      if (btn) btn.click();
    })()`,
  });
  ...
  const h = Math.min(Math.ceil(layoutRes.result.contentSize.height), 2400);
  ...
  const path = 'C:\\Users\\Tiago\\Desktop\\Globe Memories -  Github\\globeMemoriesReact\\tools\\my-travels-modal-screenshot.png';
