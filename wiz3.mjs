// Click Continuar to go to step 2 (Destinos) and check País/Cidade
...
await screenshot('C:/Users/Tiago/AppData/Local/Temp/screen-wizard-step2.png');

const fields = await evalJs(`
(() => {
  const ins = Array.from(document.querySelectorAll('input, select, textarea'));
  return ins.map(i => (i.tagName + ':' + JSON.stringify({ v: (i.value || ''), p: (i.placeholder || ''), n: (i.name || '') }).slice(0, 80))).join(' | ');
})()
`);
console.log('  fields:', fields);

const headings = await evalJs(`
(() => {
  const hs = Array.from(document.querySelectorAll('h1, h2, h3, h4'));
  return hs.map(h => h.tagName + ':' + h.innerText.slice(0, 60)).join(' | ');
})()
`);
